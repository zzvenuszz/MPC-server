...
# Expose port cho Dashboard Web UI (HF Spaces dùng port 7860)
EXPOSE 7860

# Healthcheck - kiểm tra Dashboard endpoint
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
    CMD python -c "import urllib.request; urllib.request.urlopen('http://localhost:7860/api/status', timeout=5)" || exit 1
